...
void loop() {
    ...
    card.checkautostart();
      ...
      if (abort_sd_printing) {
        ...
        clear_command_queue();
        enqueue_and_echo_command("G92 E0"); // reset the extruder axis to zero
        enqueue_and_echo_command("G1 E-3"); // do retract
        enqueue_and_echo_command("G27 P2"); // up Z by Z-park amount
        quickstop_stepper();
        print_job_timer.stop();
        //thermalManager.disable_all_heaters(); // cooldown
        #if FAN_COUNT > 0
          for (uint8_t i = 0; i < FAN_COUNT; i++) fanSpeeds[i] = 0;
        #endif
        wait_for_heatup = false;
